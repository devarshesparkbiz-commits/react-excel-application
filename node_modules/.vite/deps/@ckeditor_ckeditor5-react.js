...
function assignAttributesPropToMultiRootEditorConfig(attributes, config) {
	if (getInstalledCKBaseFeatures().rootsConfigEntry) {
		const roots = uniq([
...
function assignInitialDataToMultirootEditorConfig(data, config) {
	if (getInstalledCKBaseFeatures().rootsConfigEntry) {
		const roots = uniq([
			...
			if (rootInitialData && data?.[rootName]) console.warn(`Editor data should be provided either using \`config.roots['${rootName}'].initialData\` or the bound component property. The config value takes precedence over the bound component property and will be used when both are specified.`);
			acc[rootName] = {
				...rootConfig,
	...
	if (data && config?.initialData) console.warn("Editor data should be provided either using `config.initialData` or the bound component property. The config value takes precedence over the bound component property and will be used when both are specified.");
	return {
		...config,
...
var useMultiRootEditor = (props) => {
	...
	const _createEditor = useRefSafeCallback(async (initialData, config) => {
		...
		const editor = await (getInstalledCKBaseFeatures().elementConfigAttachment ? Editor.create({
			...mergedConfig,
		...
		if (props.disabled) editor.enableReadOnlyMode(REACT_INTEGRATION_READ_ONLY_LOCK_ID);
		const modelDocument = editor.model.document;
		const viewDocument = editor.editing.view.document;
		...
		if (shouldUpdateEditor.current) {
			...
			const _handleNewRoots = (roots2) => {
				...
				for (const rootName of roots2) {
					...
					let attrs = { isUndoable: true };
					if (supports.rootsConfigEntry) attrs = {
						...attrs,
			...
			const _updateEditorAttributes = (writer, roots2) => {
				for (const rootName of roots2) {
					for (const key of Object.keys(attributes[rootName])) instance.registerRootAttribute(key);
					const root = instance.model.document.getRoot(rootName);
					for (const key of Object.keys(instance.getRootAttributes(rootName))) writer.removeAttribute(key, root);
